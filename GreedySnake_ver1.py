...
game_start = 3
music_playing = False
config = {'DEBUG': False, 'music_sound': True, 'effect_sound': True, 'show_fps': False, 'show_score': True, 'high_scores': [0, 0, 0, 0, 0, 0, 0, 0, 0]}

# 設定畫面大小
SCREEN_WIDTH = dis_width = 1200
SCREEN_HEIGHT = dis_height = 800
SCREEN = dis = pygame.display.set_mode((dis_width, dis_height))
...
# 觸發選擇按鈕
def toggle_btn(text, x, y, w, h, click, text_colour=black, enabled=True, draw_toggle=True, enabled_color=blue_light, disabled_color=grey):
    mouse = pygame.mouse.get_pos()
...
# -------------------------------------------------------------------------
def gameLoop():
    global music_playing, snake_speed
    # 載入倒數畫面
    ...
    while True:
        ...
        if Length_of_snake % 2 == 0:
            print('加速')
            snake_speed += (Length_of_snake / 2 * 0.001)
